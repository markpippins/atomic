...
export class FileExplorerComponent implements OnDestroy {
  ...
  getSelectedNodes(): FileSystemNode[] {
    const selection = this.selectedItems();
    return this.state().items.filter(i => selection.has(i.name));
  }
  
  private getItemReference(node: FileSystemNode): { name: string, type: 'folder' | 'file' } {
    return { name: node.name, type: node.type };
  }
  
  private getSelectedItemReferences(): { name: string, type: 'folder' | 'file' }[] {
  ...
  async onFilesUploaded(files: FileList): Promise<void> {
    ...
    try {
      await Promise.all(
        Array.from(files).map(file => this.uploadFile(file))
      );
    } catch (e) {
  ...
  // Mock implementation for now, will implement later
  private async uploadFile(file: File): Promise<void> {
    // For now, just create the file with empty content
    await this.fileSystemProvider().createFile(this.providerPath(), file.name);
  }
